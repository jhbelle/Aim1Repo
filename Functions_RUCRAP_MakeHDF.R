...
AddVals <- function(baseDat, addDat, varInt, corLong=0){
  ...
  addDat$Longitude <- addDat$Longitude - corLong
  ...
  outDat <- merge(baseDat, addDat, by=c("Longitude", "Latitude."), all.x=T)
  # Add new values to appropriate column
  colVarInt = which(colnames(outDat) == varInt)
  outDat[,colVarInt] = ifelse(is.na(outDat$Value.), outDat[,colVarInt], outDat[,colVarInt] + outDat$Value.)
