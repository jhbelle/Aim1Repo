...
% -----------------------
Collocsfile = 'E://CalifG24hr.csv';
FPath = 'E://Testing/';
OutpFile1km = 'E://CalifCloudCollocs1km/';
OutpFile5km = 'E://CalifCloudCollocs5km/';
% -----------------------
...
% Pull out needed values to ID this record and find matching MODIS
    ...
    Varnames = {'State', 'County', 'Site', 'Date', 'Time', 'Row', 'Col', 'DistStation', 'Multi', 'CloudAOD', 'PCloudAOD'};
    ...
    Varnames = {'State', 'County', 'Site', 'Date', 'Time', 'LatMatch', 'LongMatch', 'DistStation', 'CloudTopHgt', 'CloudFrac'};
    formatHeader = '%s, %s, %s, %s, %s, %s, %s, %s, %s, %s\n';
    ...
    for j=1:length(filen1)
        ...
        for k=1:length(Index5km)
            Index = Index5km(k);
            rowfile = {State, County, Site, Date{:}, Time, Lat5km(Index), Long5km(Index), Dists5km(Index), TopHgt(Index), CloudFrac(Index)};
            rowform = '%i, %i, %i, %s, %s, %f, %f, %f, %f, %f\n';
            fprintf(Outp5km, rowform, rowfile{1,:});
        end
        for l=1:length(row1km)
            row = row1km(l);
            col = col1km(l);
            rowfile = {State, County, Site, Date{:}, Time, row, col, Dists1km(row, col), Multi(row,col), CloudAOT(row,col), CloudAOT_PCL(row,col)};
            rowform = '%i, %i, %i, %s, %s, %i, %i, %f, %f, %f, %f\n';
