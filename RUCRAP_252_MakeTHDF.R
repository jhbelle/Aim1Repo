...
RUCRAPloc = "/gc_runs/RUCRAP_252_Intermediate/"
OutputFolder = "/gc_runs/RUCRAP_FinalOutputs/"
# Open grid data
GridDat = read.csv("/aqua/Jess/Data/RUCRAP_252_SubGrid.csv")
...
Days = seq(as.Date("2008/12/23", "%Y/%m/%d"), as.Date("2016/01/01", "%Y/%m/%d"), "days")
for (i in seq(1, length(Days))){
  ...
  for (hour in seq(0,23)){
    ...
    if (nrow(APhrs) > 0){
      ...
      for (var in varlist){
        # Read in data
        dat = try(read.table(sprintf("%sruc2anl_252_%s_%02d_%s.txt", RUCRAPloc, as.character(day, "%Y%m%d"), hour, var), header=T)[,1:3], silent=T)
    ...
    if (hour == 2){
      ...
      if (max(AquaCounts[,3:ncol(AquaCounts)]) > 0){
        ...
        # Write HDF file
        writeHDF(AquaGridDat, sprintf("%sTerra/%d/RUCRAP_252_%s.h5", OutputFolder, Year, as.character(day-1, "%Y%m%d")))
      } 
