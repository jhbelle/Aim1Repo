...
MassRecon <- function(DatSpec, Dat24hr, DatBlanks){
  ...
  DatSpec <- merge(Dat24hr[,c(1:3,6:9,11)], DatSpec[!is.na(DatSpec$ReconMass),], by=c("State", "County", "Site", "Date")) # Only 2,586 could be matched to a gravimetric observation? Remaining gravimetric observations are missing
