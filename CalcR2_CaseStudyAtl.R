...
for (i in seq(1,10)){
  for (m in seq(1,12)){
    harvt = gam(sqrtPM ~ DailyMean.y + s(POINT_X.y.y, POINT_Y.y.y, k=10), data=CombTA[CombTA$CrossValSetT != i & CombTA$Month == m,], method="REML")
...
CombTA$PredH = rowMeans(CombTA[,c("PredA", "PredT", "HarvPred")], na.rm=T)
