...
Days = seq(as.Date("2012/01/01", "%Y/%m/%d"), as.Date("2012/04/01", "%Y/%m/%d"), "days")
  ...
  for (hour in seq(0,23)){
    ...
    if (hour == 2){
      ...
      if (max(AquaCounts[,3:ncol(AquaCounts)]) > 0){
        ...
        # Write HDF file
        writeHDF2(AquaGridDat, sprintf("%sTerra/%d/RUCRAP_130_%s.h5", OutputFolder, Year, as.character(day-1, "%Y%m%d")))
      } 
